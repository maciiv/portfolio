...
export default class CovidWorldTimelineOverlay extends React.PureComponent<CovidWorldTimelineOverlayProps, { isOn: boolean }> {
    ...
    private renderContent(x: number, data: CovidData) {
        ...
        d3.select(this.refContentHosp.current)
            .select(".item-value")
            .text(`${Math.round(data.hosp)} (${Math.round(data.hosp / data.cases * 10000) / 100}%)`)
        d3.select(this.refContentDeaths.current)
            .select(".item-value")
            .text(`${Math.round(data.deaths)} (${Math.round(data.deaths / data.cases * 10000) / 100}%)`)
        let tooltipX = x + 10
            ...
            <g transform={`translate(${this.props.location.state.translateX}, ${this.props.location.state.translateY})`}>
                <g opacity={this.state.isOn ? 1 : 0}>
                    ...
                    <g ref={this.refTooltipContent} className="tooltip-content">
                        <rect className="content-background" y={-20} x={-5} width={200} height={80} />
